...
class AllocationPolicy:
    ...
    def find_feasible_fog_nodes(self, vehicle):
        """Finds all the fog nodes that can reach the given vehicle"""
        feasible_fog_nodes = []
        for fog_node in self.fog_nodes:
            if distance(fog_node.position, vehicle.position) < fog_node.coverage_radius:
                feasible_fog_nodes.append(fog_node)
        return feasible_fog_nodes

    def allocate(self, vehicle):
        """Takes a vehicle and implements the allocation algorithm for choosing fog nodes"""
...
class SignalAwareAllocationPolicy(AllocationPolicy):
    ...
    def allocate(self, vehicle):
        ...
        best_fog_node.add_vehicle(vehicle)
